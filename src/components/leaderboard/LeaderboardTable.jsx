          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.map((row, idx) => (
                <tr
                  key={row.name}
                  className={`border-b border-border/50 hover:bg-muted/20 transition-colors ${idx < 3 ? RANK_BG[idx] : ""}`}
                >
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center w-6 h-6">
                      {idx < 3 ? RANK_ICONS[idx] : <span className="text-xs text-muted-foreground font-semibold">{idx + 1}</span>}
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`font-medium ${idx === 0 ? "text-foreground" : "text-foreground/80"}`}>{row.name}</span>
                    </div>
                  </td>
