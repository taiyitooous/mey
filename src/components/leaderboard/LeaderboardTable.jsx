...
import { Trophy, Medal, Search, GitCompare, X } from "lucide-react";
...
export default function LeaderboardTable({ data, criteria, type, loading }) {
  ...
  const [selected, setSelected] = useState([]); // up to 2 seller names
  ...
  const handleCompare = () => {
    ...
    navigate(`/seller-comparison?a=${encodeURIComponent(selected[0])}&b=${encodeURIComponent(selected[1])}`);
  };
    ...
    <div className="space-y-3">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-border bg-card text-sm">
              {selected.map((name) => (
                <span key={name} className="flex items-center gap-1.5">
                  ...
                  <button onClick={() => toggleSelect(name)} className="text-muted-foreground hover:text-foreground">
                    <X className="w-3 h-3" />
                  </button>
                </span>
              ))}
              {selected.length === 1 && (
                <span className="text-xs text-muted-foreground">· selecione mais 1</span>
              )}
              {selected.length === 2 && (
                <Button size="sm" onClick={handleCompare} className="h-6 text-xs px-2.5 ml-1 gap-1">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#2A342D]" style={{ background: "hsl(150 17% 7%)" }}>
                <th className="text-left px-3 py-3.5 text-xs font-bold text-muted-foreground uppercase tracking-wider w-8" title="Selecionar para comparar" />
                ...
                filtered.map((row) => {
                  ...
                  return (
                    <tr
                      ...
                      onMouseEnter={(e) => { if (!isSelected) e.currentTarget.style.background = `${personColor}0D`; }}
                      onMouseLeave={(e) => {
                        e.currentTarget.style.background = isSelected
                          ? `${personColor}20`
                          : isTop3 ? `${rankColor}08` : "transparent";
                      }}
                    >
                      {/* Checkbox */}
                      <td className="px-3 py-3.5">
                        <button
                          ...
                          style={{
                            borderColor: isSelected ? personColor : "hsl(142 11% 28%)",
                            background: isSelected ? personColor : "transparent",
                          }}
                        >
      ...
      {selected.length === 0 && (
        <p className="text-xs text-muted-foreground text-center pt-1">
          💡 Selecione dois vendedores na tabela para comparar lado a lado
        </p>
      )}
    </div>
  );
